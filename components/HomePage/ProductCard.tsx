...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div
      className={clsx(
        'group relative w-full cursor-pointer overflow-hidden rounded-lg border border-zinc-200 bg-slate-100 font-bold transition-all duration-500 dark:border-none dark:bg-main-600 dark:font-normal',
        {
    ...
    >
      ...
      <div className="relative">
                ...
                <CarouselItem className="!p-0" key={index}>
                  <div>
                    ...
                    >
                      <CardContent
                        className={clsx(
                          'flex aspect-square w-full items-center justify-center rounded-none !p-0',
                          variant === 'vertical'
        ...
        <div className="absolute left-4 top-4 space-y-1">
          {product.situation === 'new' ? (
            <Badge className="bg-red-600">{t('typeSwitcher.situation1')}</Badge>
          ) : (
      ...
      <div className="w-full p-5 text-zinc-800 dark:text-white">
        ...
        <div className="flex items-center justify-between gap-1.5 font-medium *:w-full *:rounded-lg *:border *:border-zinc-200 *:bg-slate-200 *:px-1 *:py-3 *:text-center *:text-zinc-700 *:dark:border-none dark:*:bg-main-700 dark:*:text-white">
          {/* Mileage */}
          <div>
            <CircleGauge className="mx-auto size-5 " />
            <p className="mt-2 text-[10px]">{product.mileage} km</p>
          </div>
          {/* Gear Type */}
          <div>
            <Cog className="mx-auto size-5" />
            <p className="mt-2 text-[10px]">{product.gearType}</p>
          </div>
          {/* Fuel Type */}
          <div>
            <Fuel className="mx-auto size-5 " />
